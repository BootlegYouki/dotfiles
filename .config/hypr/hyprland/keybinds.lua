...
local function flatten_keybinds(keybinds, keys)
    ...
    elseif keybinds ~= nil and keybinds ~= "" then
...
-- Go to workspace -1/+1
create_bind(vars.kbPrevWs, fn.relative_ws(-1), repeating_unless_mouse)
create_bind(vars.kbNextWs, fn.relative_ws(1), repeating_unless_mouse)

-- Go to workspace group -1/+1
create_bind(vars.kbPrevWsGroup, fn.relative_ws(-10), repeating_unless_mouse)
create_bind(vars.kbNextWsGroup, fn.relative_ws(10), repeating_unless_mouse)
...
for _, dir in ipairs({ "left", "right", "up", "down" }) do
    create_bind({ "SUPER + " .. dir, "SUPER + SHIFT + " .. dir }, hl.dsp.window.move({ direction = dir }))
end
...
create_bind(vars.kbDevWs, fn.toggle("dev"))
...
-- Clipboard & Universal Copy/Paste
...
local function is_terminal(win)
    ...
    for term, _ in pairs(terminal_classes) do
        if class:find(term, 1, true) then return true end
    end
    return false
end
...
local function universal_copy()
    local win = hl.get_active_window()
    local mods = is_terminal(win) and "CTRL SHIFT" or "CTRL"
...
create_bind(vars.kbCopy, universal_copy)
create_bind(vars.kbPaste, universal_paste)
...
-- Wallpaper shift (Super + , and Super + .)
create_bind(vars.kbWallpaperPrev, hl.dsp.exec_cmd("caelestia-wallpaper-shift prev"))
create_bind(vars.kbWallpaperNext, hl.dsp.exec_cmd("caelestia-wallpaper-shift next"))
