...
ColumnLayout {
    ...
    property string displayTitle: Players.active?.trackTitle ?? ""
    property string displayArtist: Players.active?.trackArtist || qsTr("Unknown artist")
    property string displayAlbum: Players.active?.trackAlbum || qsTr("Unknown album")

    function updateTrackDetails(): void {
        const title = Players.active?.trackTitle ?? "";
        const artist = Players.active?.trackArtist || "";
        const album = Players.active?.trackAlbum || "";

        if (Romaji.mode === "original" || !Romaji.enabled) {
            displayTitle = title;
            displayArtist = artist || qsTr("Unknown artist");
            displayAlbum = album || qsTr("Unknown album");
        } else {
            if (title) Romaji.convert(title, res => displayTitle = res, "romaji");
            else displayTitle = "";

            if (artist) Romaji.convert(artist, res => displayArtist = res, "romaji");
            else displayArtist = qsTr("Unknown artist");

            if (album) Romaji.convert(album, res => displayAlbum = res, "romaji");
            else displayAlbum = qsTr("Unknown album");
        }
    }

    Connections {
        target: Players
        function onActiveChanged() { root.updateTrackDetails(); }
    }

    Connections {
        target: Players.active
        function onTrackTitleChanged() { root.updateTrackDetails(); }
        function onTrackArtistChanged() { root.updateTrackDetails(); }
        function onTrackAlbumChanged() { root.updateTrackDetails(); }
    }

    Connections {
        target: Romaji
        function onModeChanged() { root.updateTrackDetails(); }
    }

    Component.onCompleted: updateTrackDetails()
    ...
    ButtonRow {
        ...
        IconButton {
            ...
            checkedRadius: Tokens.rounding.medium
            shapeMorph: true
