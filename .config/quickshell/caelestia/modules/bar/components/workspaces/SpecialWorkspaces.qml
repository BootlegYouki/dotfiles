    ...
    ListView {
        ...
        model: ScriptModel {
            values: {
                ...
                const list = Hypr.workspaces.values.filter(w => w.name.startsWith("special:") && (!GlobalConfig.bar.workspaces.perMonitorWorkspaces || w.monitor === root.monitor));
                return list.sort((a, b) => (order[a.name] ?? 99) - (order[b.name] ?? 99));
            }
        }
    ...
    Loader {
        ...
        sourceComponent: Item {
            StyledClippingRect {
                ...
                visible: view.currentItem !== null && view.currentIndex >= 0 && root.activeSpecial !== ""
