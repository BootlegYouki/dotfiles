...
if [ "$(id -u)" -eq 0 ]; then
    echo "❌ Error: Do not run this installer directly as root (makepkg/yay will refuse to build)."
...
echo "=========================================================="
echo "✨ Starting Caelestia Desktop & SDDM Fresh Installation"
echo "=========================================================="
...
# --- 3. GPU Hardware Drivers Detection ---
echo "▶ [3/9] Detecting GPU and installing display drivers..."
GPU_INFO="$(lspci | grep -Ei 'vga|3d|display' || true)"
echo "Detected graphics hardware: $GPU_INFO"
...
# --- 4. Official Core Packages & Utilities ---
echo "▶ [4/9] Installing Hyprland, audio, fonts, and system utilities..."
sudo pacman -S --noconfirm --needed \
    ...
    jq socat fd ripgrep fzf zoxide direnv eza btop cava micro python-pillow python-pip python-evdev \
    ttf-jetbrains-mono-nerd noto-fonts noto-fonts-emoji noto-fonts-cjk \
    ttf-rubik-vf ttf-roboto ttf-cascadia-code-nerd flatpak

# --- 5. SDDM & Caelestia Theme ---
echo "▶ [5/9] Installing SDDM and Caelestia Locklike theme..."
sudo pacman -S --noconfirm --needed \
    sddm qt6-declarative qt6-5compat qt6-svg qt6-multimedia

# Install Caelestia SDDM theme from AUR
if ! pacman -Qi caelestia-sddm-locklike-git &>/dev/null && ! pacman -Qi caelestia-sddm &>/dev/null; then
    ...
    $AUR_HELPER -S --noconfirm --needed caelestia-sddm-locklike-git || true
fi
...
# --- 6. Caelestia Desktop & AUR Packages ---
...
# --- 7. Restoring Dotfiles & User Configurations ---
...
# --- 8. Set Default Login Shell to Fish ---
...
# --- 9. Enable Services & Pacman Hooks ---
...
if [ -f "$DOTFILES_DIR/scripts/genshin_f_macro.py" ]; then
    ...
    chown "$TARGET_USER:$TARGET_USER" "$TARGET_HOME/genshin_f_macro.py"
fi
...
echo "=========================================================="
echo "SDDM is enabled with the Caelestia Locklike theme."
echo "Your default shell is set to Fish with Starship prompt."
echo "Hyprland is configured to launch seamlessly via UWSM."
echo ""
