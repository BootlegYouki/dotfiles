...
Rectangle {
    ...
    readonly property bool isPrimaryScreen: {
        if (config.primaryScreen && config.primaryScreen.length > 0) {
            if (config.primaryScreen.toLowerCase() === "primary") {
                return (typeof Screen.primary !== "undefined" && Screen.primary) || (Screen.virtualX === 0 && Screen.virtualY === 0);
            }
            return Screen.name === config.primaryScreen;
        }
    ...
    PowerBar {
        ...
        z: 100
        opacity: (isPrimaryScreen && container.uiReady) ? 1 : 0
    ...
    Item {
        ...
        onVisibleChanged: {
            if (visible && isPrimaryScreen) passwordField.forceActiveFocus();
        }
